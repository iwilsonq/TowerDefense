...
Animation* AnimatedSprite::getAnimation()const {return _animation;} // Simple Set/Get inline functions
...
void AnimatedSprite::setRepeat(int nb) {_repeat = nb;}
    ...
    if(_status == Playing and _animation)
    {
        ...
        if(_elapsed > _delta)   // Need to change frame
        {
            ...
            else
            {
                ...
                if(not _loop)   //Can we make another loop?
                {
                    --_repeat;
                    if (_repeat <= 0)
                    {
